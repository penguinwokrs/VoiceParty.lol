...
- **提出前に必ず確認すること**: `wrangler.toml` の
  `RIOT_VALIDATION_ENABLED` が `"false"` の間、バックエンドは Riot API を一切
  呼ばず、入力された Riot ID をそのまま受け入れる。この状態のまま提出すると、
  本書の「ACCOUNT-V1 で実在確認する」という説明が**事実と食い違う**。
  審査を受けるときは `"true"` に戻すこと（有効な API キーが前提。キーが失効
  していると全員が参加できなくなる点に注意）。
- SUMMONER-V4 が `jp1` 固定で JP 以外のアイコンが取得できなかった問題は
  #90 で修正済み（プレイヤーが選んだプラットフォームへルーティングする）。
