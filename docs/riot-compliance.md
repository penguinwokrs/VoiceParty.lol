...
> **将来（RSO / SPECTATOR-V5 連携時）の注意（#19）**: ライブマッチに基づく自動ルームは、本人が入力していない他プレイヤーを可視化・同室化するため、「可視性を伴わないプレイヤーの非匿名化」に抵触し得る。実装前に、(a) 各参加者の明示同意/オプトイン、(b) 相手チームを同室にしない、(c) 可視範囲の限定、を設計として確定する。確定するまで当該機能は実装保留とする。
...
| 項目 | 状況 |
|---|---|
| Developer Portal 登録・ステータス | 登録し、Approved / Acknowledged を維持する（[申請メモ](./riot-api-application.md) 参照）。**広告・アフィリエイトによる収益化は、Production Key 承認かつ商用利用を Riot に開示・Acknowledge 取得した後に開始する（承認前は広告を出さない運用ゲート）** |
